...
bool HitTest(const POINT& start, const POINT& end, const POINT& p, double d)
{
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		if (nPoint < 4) {
			point[nPoint] = { GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) };
			nPoint++;
			InvalidateRect(hWnd, 0, TRUE);
		}
		else
		{
			...
			if (BeginPath(hdc))
			{
				PolyBezier(hdc, (LPPOINT)point, nPoint);
				if (EndPath(hdc))
				{
					if (FlattenPath(hdc)) {
						int nNumPoints = GetPath(hdc, (LPPOINT)NULL, (LPBYTE)NULL, 0);
						if (nNumPoints > 0)
						{
							LPPOINT lpPt = (LPPOINT)malloc((DWORD)(sizeof(POINT) * nNumPoints));
							LPBYTE lpB = (LPBYTE)malloc((DWORD)(sizeof(BYTE) * nNumPoints));
							nNumPoints = GetPath(hdc, lpPt, lpB, nNumPoints);
							POINT pt = { GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) };
							for (int nPt = 0; nPt < nNumPoints - 1; nPt++)
							{
								if (HitTest(lpPt[nPt], lpPt[nPt + 1], pt, POINT2PIXEL(8))) {
									MessageBeep(0);
									select = true;
								}
							}
	...
	case WM_PAINT:
	{
		...
		for (int n = 0; n < nPoint; n++) {
			Ellipse(hdc, point[n].x - 4, point[n].y - 4, point[n].x + 4, point[n].y + 4);
		}
		if (nPoint == 4) {
			HPEN hPen = CreatePen(PS_SOLID, select ? 4 : 1, select ? RGB(255, 0, 0) : RGB(0, 0, 0));
			HPEN hOldPen = (HPEN)SelectObject(hdc, hPen);
			PolyBezier(hdc, point, nPoint);
